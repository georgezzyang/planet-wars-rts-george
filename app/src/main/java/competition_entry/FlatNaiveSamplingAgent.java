...
/**
 * Flat NaiveSampling for Planet Wars.
 *
 * NOTE on naming: this is NOT full NaiveMCTS (Ontanon 2013). NaiveMCTS = NaiveSampling
 * action selection embedded in a UCT TREE. This class implements only the NaiveSampling
 * selection mechanism, applied at the root with no tree (one-level lookahead via
 * heuristic rollout). True tree-based NaiveMCTS lives in {@link NaiveMCTSAgent}.
 *
 * Kept around as a baseline + ablation: lets us measure how much value the tree adds
 * over pure root-level NaiveSampling.
 *
 * Three MABs (NaiveSampling at the root):
 ...
 *
 * Epsilon-greedy at the top: with prob epsilon explore via the local MABs (lets new
 * pairs appear), else exploit the best global arm so far.
 */
public class FlatNaiveSamplingAgent extends PlanetWarsPlayer {
